...
export const files = pgTable('files', {
  ...
  storageBackend: varchar('storage_backend', { length: 20 }).notNull().default('s3'),
...
export const items = pgTable(
  ...
  (table) => [
    ...
    check('items_status_check', sql`${table.status} IN ('active', 'archived', 'superseded')`),
...
export const links = pgTable(
  ...
  (table) => [
    unique('links_unique_evidence_entity').on(table.evidenceId, table.entityType, table.entityId),
