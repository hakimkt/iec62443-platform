...
export const engagements = pgTable(
  ...
  (table) => [
    ...
    check('engagements_target_sl_check', sql`${table.targetSl} BETWEEN 0 AND 4`),
    check('engagements_current_sl_check', sql`${table.currentSl} BETWEEN 0 AND 4`),
    check(
...
export const responses = pgTable(
  ...
  (table) => [
    ...
    check('responses_maturity_level_check', sql`${table.maturityLevel} BETWEEN 0 AND 4`),
    unique('responses_unique_engagement_question').on(table.engagementId, table.questionId),
...
export const scorecards = pgTable('scorecards', {
  ...
  currentSl: smallint('current_sl'),
  targetSl: smallint('target_sl'),
  gap: smallint('gap').generatedAlwaysAs(sql`(target_sl - current_sl)`),
