...
const badgeVariants = cva(
  'inline-flex items-center font-medium whitespace-nowrap transition-colors',
  {
    variants: {
      ...
      securityLevel: {
        SL0: 'bg-slate-200 text-slate-700 dark:bg-slate-800 dark:text-slate-300',
        SL1: 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300',
        SL2: 'bg-blue-200 text-blue-800 dark:bg-blue-900/40 dark:text-blue-200',
        SL3: 'bg-violet-100 text-violet-700 dark:bg-violet-900/40 dark:text-violet-300',
        SL4: 'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300',
      },
      outline: {
    ...
    compoundVariants: [
      ...
      },
      {
        ...
        outline: true,
        className:
          'border-surface-200 text-surface-500 dark:border-surface-600 dark:text-surface-400',
      },
      {
        status: 'cancelled',
        outline: true,
      ...
      },
      {
        ...
        outline: true,
        className: 'border-slate-200 text-slate-600 dark:border-slate-600 dark:text-slate-400',
      },
      ...
      },
      {
        securityLevel: 'SL2',
        outline: true,
        className: 'border-blue-300 text-blue-800 dark:border-blue-600 dark:text-blue-200',
      },
      {
        securityLevel: 'SL3',
        outline: true,
  ...
  (
    ...
    return (
      <Comp
        ref={ref}
        className={cn(
          badgeVariants({ size, status, severity, securityLevel, outline, dot }),
          className,
        )}
        {...props}
      >
        {dot && dotColor && (
          <span className={cn('shrink-0 rounded-full', dotSizeMap[size ?? 'md'], dotColor)} />
        )}
        {children}
      </Comp>
    );
