          ...
          <div className="mt-2 flex items-center gap-3">
            <Badge
              variant={
                risk.status === 'closed'
                  ? 'archived'
                  : risk.status === 'monitored' || risk.status === 'accepted'
                    ? 'completed'
                    : 'in_progress'
              }
              size="sm"
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            {isEditing ? (
              <div className="mt-4 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <Select
                      value={String(formState['category'] ?? '')}
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    value={String(formState['threatSource'] ?? '')}
                    onChange={(e) => setFormState((s) => ({ ...s, threatSource: e.target.value }))}
                  />
                </div>
                <div className="space-y-1">
                  <Label>Vulnerability</Label>
                  <Input
                    value={String(formState['vulnerability'] ?? '')}
                    onChange={(e) => setFormState((s) => ({ ...s, vulnerability: e.target.value }))}
                  />
                </div>
              </div>
            ) : (
              <dl className="mt-4 space-y-3">
                <div className="flex justify-between">
                  ...
                  <dd className="text-sm text-surface-900">{risk.title}</dd>
                </div>
                <Separator />
                <div className="flex justify-between">
                  <dt className="text-sm text-surface-500">Description</dt>
                  <dd className="text-sm text-surface-900 max-w-[60%] text-right">
                    {risk.description || '—'}
                  </dd>
                </div>
                <Separator />
                ...
                <Separator />
                <div className="flex justify-between">
                  <dt className="text-sm text-surface-500">Threat Source</dt>
                  <dd className="text-sm text-surface-900">{risk.threatSource || '—'}</dd>
                </div>
                <Separator />
                <div className="flex justify-between">
                  <dt className="text-sm text-surface-500">Vulnerability</dt>
                  <dd className="text-sm text-surface-900">{risk.vulnerability || '—'}</dd>
                </div>
                <Separator />
                <div className="flex justify-between">
                  <dt className="text-sm text-surface-500">Status</dt>
                  <dd className="text-sm text-surface-900 capitalize">{risk.status}</dd>
                </div>
              </dl>
            )}
          </div>
          ...
          <div className="rounded-lg border border-surface-200 bg-surface-0 p-6">
            ...
            {isEditing ? (
              <div className="mt-4 space-y-3">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <Select
                      value={String(formState['likelihood'] ?? '')}
                      onValueChange={(v) =>
                        setFormState((s) => ({ ...s, likelihood: parseInt(v, 10) }))
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
                  ...
                  <div className="space-y-1">
                    ...
                    <Select
                      value={String(formState['impact'] ?? '')}
                      onValueChange={(v) =>
                        setFormState((s) => ({ ...s, impact: parseInt(v, 10) }))
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
                ...
                <div className="space-y-1">
                  ...
                  <Select
                    value={String(formState['treatment'] ?? '')}
                    onValueChange={(v) => setFormState((s) => ({ ...s, treatment: v }))}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    value={String(formState['iecRequirement'] ?? '')}
                    onChange={(e) =>
                      setFormState((s) => ({ ...s, iecRequirement: e.target.value }))
                    }
                ...
                <div className="space-y-1">
                  ...
                  <Input
                    type="date"
                    value={String(formState['reassessBy'] ?? '')}
                    onChange={(e) => setFormState((s) => ({ ...s, reassessBy: e.target.value }))}
                  />
                </div>
              </div>
            ) : (
              <dl className="mt-4 space-y-3">
                <div className="flex justify-between">
                  <dt className="text-sm text-surface-500">Likelihood</dt>
                  <dd className="text-sm text-surface-900">{risk.likelihood}</dd>
                </div>
                <Separator />
                <div className="flex justify-between">
                  <dt className="text-sm text-surface-500">Impact</dt>
                  <dd className="text-sm text-surface-900">{risk.impact}</dd>
                </div>
                <Separator />
                <div className="flex justify-between">
                  <dt className="text-sm text-surface-500">Inherent Score</dt>
                  <dd className="text-sm font-semibold text-surface-900">{risk.inherentScore}</dd>
                </div>
                <Separator />
                <div className="flex justify-between">
                  <dt className="text-sm text-surface-500">Residual Score</dt>
                  <dd className="text-sm text-surface-900">{risk.residualScore ?? '—'}</dd>
                </div>
                <Separator />
                <div className="flex justify-between">
                  <dt className="text-sm text-surface-500">Treatment</dt>
                  <dd className="text-sm text-surface-900 capitalize">{risk.treatment}</dd>
                </div>
                <Separator />
                <div className="flex justify-between">
                  <dt className="text-sm text-surface-500">IEC Requirement</dt>
                ...
                <Separator />
                <div className="flex justify-between">
                  <dt className="text-sm text-surface-500">Reassess By</dt>
                  <dd className="text-sm text-surface-900">
                    {risk.reassessBy ? new Date(risk.reassessBy).toLocaleDateString() : '—'}
                  </dd>
                </div>
                <Separator />
                <div className="flex justify-between">
                  <dt className="text-sm text-surface-500">Identified At</dt>
                  <dd className="text-sm text-surface-900">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewTreatment((t) => ({ ...t, type: e.target.value }))}
                    placeholder="e.g. Safeguard implementation"
                  />
                </div>
                <div className="space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewTreatment((t) => ({ ...t, targetDate: e.target.value }))}
                  />
                </div>
              </div>
              <div className="space-y-1">
                ...
                <Textarea
                  ...
                  onChange={(e) => setNewTreatment((t) => ({ ...t, description: e.target.value }))}
                  rows={2}
                />
              </div>
              <div className="space-y-1">
                ...
                <Input
                  ...
                  onChange={(e) => setNewTreatment((t) => ({ ...t, costEstimate: e.target.value }))}
                  placeholder="0.00"
                />
              </div>
              <div className="flex items-center justify-end gap-2">
                <Button variant="secondary" size="sm" onClick={() => setShowNewTreatment(false)}>
                  Cancel
                </Button>
                <Button
                  variant="primary"
                  ...
                  disabled={!newTreatment.type || !newTreatment.description}
                >
                  Create
                </Button>
              </div>
          ...
          {treatments && treatments.length > 0
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={cn(
                          'text-xs font-medium px-2 py-0.5 rounded',
                          treatmentStatusColors[treatment.status] ??
                  ...
                  <div className="text-right">
                    {treatment.targetDate && (
                      <p className="text-xs text-surface-500">
                        Target: {new Date(treatment.targetDate).toLocaleDateString()}
                      </p>
                    )}
                    {treatment.costEstimate !== null && treatment.costEstimate !== undefined && (
                      <p className="text-xs text-surface-500">
                        Cost: ${treatment.costEstimate.toLocaleString()}
                      </p>
                    )}
                  </div>
                </div>
            ...
            : !showNewTreatment && (
                <p className="py-8 text-center text-sm text-surface-500">
                  No treatments configured for this risk.
                </p>
              )}
          ...
          {acceptances && acceptances.length > 0 && (
            <div className="mt-6">
              ...
              {acceptances.map((acceptance) => (
                <div
                  key={acceptance.id}
                  className="rounded-lg border border-surface-200 bg-surface-0 p-4 mb-3"
                >
