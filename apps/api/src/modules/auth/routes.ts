  ...
  app.post(
    '/forgot-password',
    {
      schema: {
        ...
        description:
          'Initiates a password reset flow by sending a reset token to the registered email.',
        body: {
          type: 'object',
          required: ['email'],
          properties: {
  ...
  app.post(
    '/mfa/setup',
    {
      schema: {
        ...
        description:
          'Generates a TOTP secret and QR code URI for MFA enrollment. Requires authentication.',
        security: [{ bearerAuth: [] }],
        response: { 200: responseSchema },
      },
      preHandler: [app.authenticate],
