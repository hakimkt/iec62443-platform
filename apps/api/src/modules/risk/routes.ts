...
export async function riskRoutes(app: FastifyInstance, options: RiskRouteOptions) {
  const { db } = options;
      ...
      schema: {
        ...
        querystring: {
          ...
          properties: {
            ...
            treatment: {
              type: 'string',
              enum: ['mitigate', 'transfer', 'accept', 'avoid', 'pending'],
            },
            status: {
              type: 'string',
              enum: ['identified', 'analyzed', 'treated', 'monitored', 'closed', 'accepted'],
            },
            search: { type: 'string', maxLength: 200 },
  ...
  app.patch(
    '/risks/:id/treatments/:treatmentId',
    {
      schema: {
        tags: ['Risks'],
