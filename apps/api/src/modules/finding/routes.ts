...
export async function findingRoutes(
    ...
    schema: {
      ...
      querystring: {
        ...
        properties: {
          ...
          status: { type: 'string', enum: ['draft', 'open', 'acknowledged', 'remediation_planned', 'in_progress', 'verification', 'verified', 'closed', 'false_positive', 'risk_accepted'] },
  ...
  // ── PATCH /findings/:id ──────────────────────────────────────────────
  app.patch('/findings/:id', {
    schema: {
      tags: ['Findings'],
      summary: 'Update a finding',
      description: 'Updates a security finding. Only draft or open findings can be updated. Requires authentication.',
      security: [{ bearerAuth: [] }],
      params: {
        type: 'object',
        required: ['id'],
  ...
  app.post('/findings/:id/transition', {
    schema: {
      tags: ['Findings'],
      summary: 'Transition finding status',
      description: 'Transitions a finding to a new status. The transition must be valid per the status workflow. Requires authentication.',
      security: [{ bearerAuth: [] }],
      params: {
        type: 'object',
        required: ['id'],
      ...
      body: {
        ...
        required: ['toStatus'],
        properties: {
          toStatus: { type: 'string', enum: ['draft', 'open', 'acknowledged', 'remediation_planned', 'in_progress', 'verification', 'verified', 'closed', 'false_positive', 'risk_accepted'] },
  ...
  app.get('/findings/:id/history', {
    schema: {
      tags: ['Findings'],
      summary: 'Get finding status history',
      description: 'Returns the status transition history for a finding. Requires authentication.',
      security: [{ bearerAuth: [] }],
      params: {
        type: 'object',
        required: ['id'],
