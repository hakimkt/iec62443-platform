...
export async function zoneRoutes(app: FastifyInstance, options: ZoneRouteOptions) {
  const { db } = options;
      ...
      schema: {
        ...
        querystring: {
          ...
          properties: {
            ...
            conduitType: {
              type: 'string',
              enum: ['hardwired', 'network', 'wireless', 'removable_media', 'human', 'other'],
            },
            securityLevel: { type: 'integer', minimum: 0, maximum: 4 },
            search: { type: 'string', maxLength: 200 },
          },
